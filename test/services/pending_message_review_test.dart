...
void main() {
  group('PendingMessageReviewService', () {
    ...
    test('approve credits existing customer and marks processed', () async {
      ...
      expect(
        audit.logs.any((l) => l.action == 'pending_message_approved'),
        isTrue,
      );
    });
    ...
    test('reject marks rejected and audits', () async {
      ...
      expect(
        audit.logs.any((l) => l.action == 'pending_message_rejected'),
        isTrue,
      );
    });
...
final class _FakeCustomers implements CustomerRepository {
  ...
  Future<Result<Customer?>> findByIdentifier(String value) async {
    final id = byIdentifier[value];
    return Success(id == null ? null : store[id]);
  }
  ...
  Future<Result<void>> saveIdentifier(CustomerIdentifier identifier) async {
    byIdentifier[identifier.value] = identifier.customerId;
    return const Success(null);
  }
...
final class _FakeCustomerService implements CustomerService {
  ...
  @override
  Future<Result<void>> blacklist(String customerId) async =>
      const Success(null);
