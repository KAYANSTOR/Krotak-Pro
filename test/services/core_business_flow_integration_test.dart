...
void main() {
  ...
  Future<void> seedCategoryAndCard({
    required int minorUnits,
    String categoryId = 'cat-200',
    ...
    final imported = await catalogService.importCards(
      ...
      drafts: [
        CardImportDraft(
          serialNumber: 'SN-$cardId',
          secretCode: 'SECRET-$cardId',
        ),
      ],
    );
  ...
  ParsedTransfer transfer(String id, int amount, {String? reference}) =>
      ParsedTransfer(
        messageId: id,
        amount: Money(minorUnits: amount, currencyCode: 'YER'),
        customerIdentifier: '733000000',
  ...
  test('transfer amount 200 selects category 200, sells one card, delivers once, and records ledger/audit', () async {
    ...
    expect(
      (audits as Success<List<AuditLog>>).value.any(
        (item) => item.action == 'sms_delivery_succeeded',
      ),
      isTrue,
    );
  });
  ...
  test('unmatched transfer amount is rejected without credit, reservation, sale, or delivery', () async {
    ...
    expect((result as Failure<Transaction>).error.code, 'unmatched_amount');
    ...
    expect(
      (audits as Success<List<AuditLog>>).value.any(
        (item) => item.action == 'transfer_unmatched_amount',
      ),
      isTrue,
    );
  });
  ...
  test('two concurrent operations can consume the single card at most once', () async {
    ...
    final results = await Future.wait([
      processor.process(transfer('m6', 200, reference: 'CONCURRENT-A')),
      processor.process(transfer('m6', 200, reference: 'CONCURRENT-B')),
    ]);
    final cardRows = await cards.findByCategory('cat-200');
  ...
  Future<Result<void>> send({
    ...
    if (fail) {
      return const Failure<void>(
        AppFailure(code: 'sms_send_failed', message: 'SMS send failed'),
      );
    }
