...
class MainActivity : FlutterActivity(), SmsListener {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, notificationMethodChannelName)
            .setMethodCallHandler { call, result ->
                when (call.method) {
        ...
        EventChannel(flutterEngine.dartExecutor.binaryMessenger, notificationEventChannelName)
            .setStreamHandler(object : EventChannel.StreamHandler {
    ...
    private fun PendingNotification.toMap(): Map<String, Any?> = mapOf(
        "id" to id,
    ...
    override fun onSmsReceived(sender: String, body: String, timestampMillis: Long) {
        runOnUiThread { eventSink?.success(mapOf("sender" to sender, "body" to body, "timestampMillis" to timestampMillis)) }
    }
    ...
    private fun requestSmsPermissions() {
        ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.RECEIVE_SMS, Manifest.permission.READ_SMS, Manifest.permission.SEND_SMS), REQUEST_SMS)
    }
