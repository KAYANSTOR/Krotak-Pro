...
/// Bottom sheet: stock by category — real Repository data only.
/// UI aligned with Kotlin CardStockSheet + product screenshot (low-stock badge).
class CardStockSheet extends StatefulWidget {
  ...
  /// Shows modal sheet; loads categories + card counts from AppScope.
  static Future<void> show(BuildContext context, {required VoidCallback onGoToCards}) {
  ...
  Future<void> _load() async {
    ...
    final rows = categories
        ...
        .map((cat) {
          ...
          final t = a + countFor(reservedList, cat.id) + countFor(soldList, cat.id);
          return _CategoryStock(category: cat, available: a, total: t);
        })
        .toList();
          ...
          children: [
            ...
            if (_loading)
              const Padding(
                ...
                child: AsyncLoadingView(message: 'جاري تحميل المخزون…'),
              )
            else if (_error != null)
              Padding(
              ...
              Flexible(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) => _CategoryStockTile(row: _rows[i]),
                ),
              ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text(
                        '${_rows.fold<int>(0, (s, e) => s + e.available)} متوفر من ${_rows.fold<int>(0, (s, e) => s + e.total)}',
