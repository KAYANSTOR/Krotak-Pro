...
class _RenewSubscriptionScreenState extends State<RenewSubscriptionScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _loading = false;
      _current = r is Success<License>
          ? '${r.value.status.name} · ينتهي ${r.value.expiresAt}'
  ...
  Future<void> _extend(int days) async {
    ...
    setState(() {
      _busy = false;
      _status = r is Success<License> ? 'تم التجديد حتى ${r.value.expiresAt}' : (r as Failure).error.message;
    });
    await _load();
  }
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                FilledButton(onPressed: _busy ? null : () => _extend(30), child: const Text('تجديد 30 يومًا (محلي)')),
                const SizedBox(height: 8),
                OutlinedButton(onPressed: _busy ? null : () => _extend(365), child: const Text('تجديد سنة (محلي)')),
