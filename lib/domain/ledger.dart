...
Money sumCompletedLedger({required List<Transaction> transactions, required String currencyCode}) {
  var total = 0;
  for (final transaction in transactions) {
    if (transaction.status != TransactionStatus.completed) continue;
    if (transaction.amount.currencyCode != currencyCode) throw const MixedCurrencyLedger();
