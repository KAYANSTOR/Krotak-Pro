...
final class DefaultPosTemplatesSeeder {
  ...
  static bool _needsKnownMigration(TransferTemplate existing, _TplSpec spec) {
    final p = existing.pattern.trim();
    // Legacy broken customer template that mixed qty/dest into customer delivery.
    if (spec.variant == 'cards-to-pos-customer' &&
        p == '{qty} كرت {amount} {dest}') {
      return true;
    }
    // Reversed qty/amount order on the built-in cards-to-pos template.
    if (spec.variant == 'cards-to-pos' &&
        (p == '{amount} كرت {qty}' || p == '{amount} كرت{qty}')) {
      return true;
    }
    return false;
  }
