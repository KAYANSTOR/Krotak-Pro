...
/// - completed ledger rows of source are re-attributed to target via new
///   compensating entries is **out of scope**; this merge only moves identity.
///   Callers that need balance consolidation must settle/credit explicitly.
/// - operation is idempotent if source already merged into the same target
final class LocalAccountMergeService {
  ...
  Future<Result<Customer>> merge({
    ...
    return unitOfWork.run(() async {
      ...
      // Idempotent: already merged into this target
