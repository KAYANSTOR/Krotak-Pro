...
final class LocalTransferProcessor implements TransferProcessor {
  ...
  Future<Result<Transaction>> _processCommercial({
    required IncomingMessage message,
    required ParsedTransfer transfer,
    required CustomerIdentityResolution resolution,
  }) async {
    final customer = resolution.customer!;
    final deliveryPhone = (resolution.deliveryPhone ?? transfer.customerIdentifier).trim();
    if (deliveryPhone.isEmpty) {
      final failure = AppFailure(
        code: 'delivery_phone_required',
        message: 'Delivery phone is required for card SMS',
      );
      await _persistTerminalFailure(
        messageId: message.id,
        status: MessageProcessingStatus.rejected,
        action: 'transfer_no_delivery_phone',
        error: failure,
        transfer: transfer,
    ...
    if (existingLedger is Success<Transaction?> && existingLedger.value != null) {
      await messages.updateStatus(message.id, MessageProcessingStatus.processed);
      return Success<Transaction>(existingLedger.value!);
    }

    // Continue commercial path — full body from known-good source
    // (placeholder truncated for this intermediate write — will complete in next push)
    return const Failure<Transaction>(
      AppFailure(code: 'commercial_path_incomplete', message: 'Use full restore'),
    );
  }

  String _operationId(ParsedTransfer transfer) =>
      transfer.reference.isNotEmpty ? transfer.reference : transfer.messageId;
