...
final class LocalTransferProcessor implements TransferProcessor {
  ...
  bool _looksLikePosCardOrder(String raw) {
    var normalized = raw
        .trim()
        .replaceAll(RegExp(r'[\u200e\u200f\u202a-\u202e\u2066-\u2069]'), '')
    ...
    return RegExp(
      r'^\d{1,2}\s+كرت\s+\d+(?:[.,]\d{1,2})?(?:\s+\+?\d{7,15})?  bool get _configured =>
      categories != null &&
      cards != null &&
      inventory != null &&
      transactions != null &&
  ...
  Future<Result<Transaction>> process(ParsedTransfer transfer) async {
    ...
    final posSemanticOrder = transfer.kind == ParsedTransferKind.posCardOrder ||
        transfer.kind == ParsedTransferKind.posInstantCharge;
    final isPosOrder = posSemanticOrder &&
        posAccount != null &&
        posAccount.status == PointOfSaleStatus.active &&
        (transfer.posId == null || transfer.posId == posAccount.posId);

    // A POS card-order-shaped message is never a financial deposit. If it
    // reaches the processor without an explicit POS-order semantic (for
    // example because the POS request template was disabled), stop it here
    // before customer resolution, balance credit, or any other financial path.
    if (_looksLikePosCardOrder(message.body) && !isPosOrder) {
      const failure = AppFailure(
        code: 'pos_order_template_required',
        message: 'رسالة طلب كروت لنقطة البيع تحتاج قالب طلب POS نشط',
      );
      await _persistTerminalFailure(
        messageId: message.id,
        status: MessageProcessingStatus.rejected,
        action: 'pos_order_blocked_as_financial_transfer',
        error: failure,
        transfer: transfer,
      );
