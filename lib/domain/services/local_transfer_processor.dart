...
final class LocalTransferProcessor implements TransferProcessor {
  ...
  Future<Result<Transaction>> process(ParsedTransfer transfer) async {
    ...
    // Phase 3: commit sale + ledger BEFORE SMS. Failure to send must not
    // release the card or allow a second voucher for the same operation.
    final completed = await saleCompleter.completeReservedSale(
      customerId: customer.id,
      cardId: card.id,
      reservationId: reservationId,
    ...
    if (sent is Failure<void>) {
      // Sale is already committed — queue for delivery retry (no second card).
