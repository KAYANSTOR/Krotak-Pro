...
/// Operator review for messages left in [MessageProcessingStatus.parsed]
/// (PD-07 Q4: unmatched amount → معلّقة → اعتماد أو رفض).
///
/// **Approve:** credit the resolved/created customer; mark processed; audit.
/// Does **not** reserve or send a card (no matching category by definition).
///
/// **Reject:** mark rejected; audit; message leaves the pending list.
final class PendingMessageReviewService {
  ...
  /// Messages awaiting operator review (parsed; not processed/rejected).
  Future<Result<List<IncomingMessage>>> listPending() async {
    ...
    final list = List<IncomingMessage>.of(
      (result as Success<List<IncomingMessage>>).value,
    );
  ...
  Future<Result<Transaction>> approve(String messageId) async {
    ...
    if (message.status == MessageProcessingStatus.processed) {
      return const Failure(
        AppFailure(
          ...
          message: 'Message was already approved',
        ),
      );
    }
    if (message.status == MessageProcessingStatus.rejected) {
      return const Failure(
        AppFailure(
          code: 'message_already_rejected',
          message: 'Message was already rejected',
        ),
      );
    }
    ...
    // PD-07 Q4: create customer if needed before deposit.
    if (!resolution.isResolved || resolution.customer == null) {
        ...
        return Failure(
          AppFailure(
            ...
            message: resolution.reasonMessage ??
                'Cannot approve without a resolvable customer',
          ),
        );
      }
      final created = await customerService.create(
        displayName: transfer.customerIdentifier,
      ...
      resolution = CustomerIdentityResolution.resolved(
        ...
        matchedIdentifier: transfer.customerIdentifier,
      );
    }

    final customer = resolution.customer!;
    final creditRef = transfer.reference.trim().isNotEmpty
        ? 'pending-approve:${transfer.reference.trim()}'
        : 'pending-approve:$messageId';
    ...
    await auditLogs.append(
      AuditLog(
        ...
        entityType: 'message',
        entityId: messageId,
        action: 'pending_message_approved',
        occurredAt: clock.now(),
        payloadJson:
            '{"transactionId":"${tx.id}","customerId":"${customer.id}","amount":${transfer.amount.minorUnits},"currency":"${transfer.amount.currencyCode}","reference":"${transfer.reference}","sender":"${message.sender}"}',
      ),
    );
    return Success(tx);
  }
  ...
  Future<Result<void>> reject(String messageId, {String? reason}) async {
    ...
    if (message.status == MessageProcessingStatus.processed) {
      return const Failure(
        AppFailure(
          code: 'message_already_processed',
          message: 'Processed messages cannot be rejected',
        ),
      );
    }
    if (message.status == MessageProcessingStatus.rejected) {
    ...
    final safeReason = (reason ?? 'رفض من شاشة الرسائل المعلّقة')
        .replaceAll('"', '\\"');
    await auditLogs.append(
      AuditLog(
        id: ids.next('audit'),
        ...
        occurredAt: clock.now(),
        payloadJson:
            '{"reason":"$safeReason","sender":"${message.sender}","previousStatus":"${message.status.name}"}',
      ),
    );
    return const Success(null);
  }
