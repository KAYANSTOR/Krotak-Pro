...
final class LocalPosAutoSettlementService {
  ...
  Future<bool> _enabled() async {
    final found = await settings.find(SettingKeys.autoPosSettlementEnabled);
    if (found is Failure<AppSetting?>) {
      return SettingDefaults.autoPosSettlementEnabled;
    }
    return SettingBool.read(
      (found as Success<AppSetting?>).value?.value,
  ...
  Future<Result<PosAccount?>> _resolveAccount(ParsedTransfer transfer) async {
    final keys = <String>{
      ...
      if (transfer.rawIdentifier != null) transfer.rawIdentifier!,
    };
    PosAccount? hit;
    for (final key in keys) {
      final found = await posRegistry.findByIdentifier(key);
      if (found is Failure<PosAccount?>) return found;
      final account = (found as Success<PosAccount?>).value;
