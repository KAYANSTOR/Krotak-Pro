...
final class LocalMessageParser implements MessageParser {
  ...
  /// Matches the two built-in POS card-order templates.
  /// Accepts both normal and reversed numeric order:
  /// 10 كرت 100, 100 كرت 10, and their Eastern-Arabic digit variants.
  ParsedTransfer? _tryPosCardTemplateMatch(
    ...
    final match = isToCustomer
        ? RegExp(r'^(\d+)\s+كرت\s+(\d+)\s+(\+?[\d]{7,15})').firstMatch(body)
        : RegExp(r'^(\d+)\s+كرت\s+(\d+)').firstMatch(body);
    if (match == null || match.end != body.length) return null;
    ...
    final destination = isToCustomer
    ...
    return ParsedTransfer(
      messageId: messageId,
      amount: Money(
        minorUnits: resolved.amountMinor,
      ...
      deliveryOverride: destination,
      instantCharge: false,
    );
  }

  ({int quantity, int amountMinor})? _resolvePosQuantityAndAmount(
    int first,
    int second,
  ) {
    ...
    if (firstLooksQty && !secondLooksQty) {
      return (quantity: first, amountMinor: second * 100);
    }
    if (!firstLooksQty && secondLooksQty) {
      return (quantity: second, amountMinor: first * 100);
    }
    return null;
  }
