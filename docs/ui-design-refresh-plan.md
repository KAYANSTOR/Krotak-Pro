...
## 3.2) تحقق CI النهائي (2026-09-18)

`analyze-test` و`build-apk` نجحا معًا: تحليل نظيف، وكل الاختبارات نجحت (203)،
وبناء APK للإصدار ونشر GitHub Release.

ثلاثة أخطاء تخطيط حقيقية كانت مخفية خلف تحذير التحليل الحاجب، وكلها أُصلحت في طبقة
العرض فقط:

| الخطأ | السبب | الإصلاح |
|---|---|---|
| `BoxConstraints forces an infinite height` في ورقة المظهر | صفّ `stretch` داخل تمرير عمودي بلا ارتفاع محدود | `IntrinsicHeight` حول الصف |
| تجاوز الحالة الفارغة/الخطأ على الشاشات القصيرة | عمود ثابت الارتفاع داخل `Center` غير قابل للتمرير | غلاف `_ScrollableCenter` مشترك |
| تجاوز ورقة الإجراءات السريعة بـ180 بكسل | ستة إجراءات في عمود ثابت داخل ورقة غير `isScrollControlled` | ورقة متحرّكة + قائمة قابلة للتمرير |
