...
- قالب `SettingKeys.promotionRewardSmsTemplate` وإرسال بعد الصرف دون إلغاء البيع.
...
- تقدم العرض يخصم عكس المبيعات فقط.
- `reclaimUnqualified` بعد `reverseSale` يعيد كرت المكافأة غير المستحق.
- تركيب الصرف في `AppContainer` + `LocalSaleService`.
