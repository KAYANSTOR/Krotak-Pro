...
- بناءات `pull_request` تستخدم مفتاح توقيع مؤقتاً خاصاً بالـCI فقط لإنتاج APK للتحقق والاختبار.
- هذا الـAPK ليس بديلاً عن الإصدار التجاري الموقّع بالمفتاح الثابت، ولا يُستخدم لتحديث النسخ التجارية المثبتة.
- بناءات `main` و`workflow_dispatch` تبقى مرتبطة بالمفتاح الثابت الموجود في GitHub Secrets.
